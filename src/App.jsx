  ...
  const DiscordTextStyler = () => {
    ...
    // New function to reset all formatting
    ...
    // Improved function to remove formatting from selected text
    const removeFormatting = () => {
      ...
      // Find the parent span of the selected text
      let parentSpan = selection.anchorNode.parentElement;
      
      // If the parent is not a span, try to find the closest span
      if (parentSpan.nodeName !== 'SPAN') {
        parentSpan = selection.anchorNode.closest('span');
      }
      ...
      // Reset active styles if no other formatting remains
      const content = textareaRef.current;
      const hasAnyFormatting = Array.from(content.getElementsByTagName('span')).length > 0;
      if (!hasAnyFormatting) {
        setActiveStyles({
          bold: false,
          italic: false,
        ...
        <Container size="md" p="md">
          <Paper shadow="md" p="lg" radius="md">
            <Stack>
              ...
              <Group justify="center" mb="md">
                <Tooltip label="Remove Formatting from Selected Text" position="bottom">
                  <Button 
                ...
                <Tooltip label="Reset All Formatting" position="bottom">
                  <Button 
